...
export const Toggle = (profile_id, { id }, body) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // (both security switches) as part of the write, so a flip of either
      // switch is enforced on the very next API or MCP call.
      const updated = await WrikeCredentials.Update(profile_id, id, updates);

      // Environments also feed the in-memory credential cache
      // (getCachedWrikeCredentials / getCachedVisibleWrikeCredentials) that
      // Wrike-facing requests read from. Separate cache, same idea.
